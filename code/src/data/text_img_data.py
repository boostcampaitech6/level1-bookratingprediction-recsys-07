...
def text_data_split(args, data):
    ...
    """
    X_train, X_valid, y_train, y_valid = train_test_split(
                                                        data['text_train'][['user_id', 'isbn', 'item_summary_vector']],
                                                        data['img_train']['img_vector'],
...
class Text_Img_Dataset(Dataset):
    def __init__(self, user_isbn_vector, img_vector, text_vector, label):
        """
        Parameters
        ----------
        user_isbn_vector : np.ndarray
    ...
    def __getitem__(self, i):
        return {
                'user_isbn_vector' : torch.tensor(self.user_isbn_vector[i], dtype=torch.long),
                'img_vector' : torch.tensor(self.img_vector[i], dtype=torch.float32),
                'text_vector' : torch.tensor(self.text_vector[i],dtype=torch.float32),
                'label' : torch.tensor(self.label[i], dtype=torch.float32),
                }
        

def text_img_data_load(args):
    """
    Parameters
    ----------
    Args : argparse.ArgumentParser
...
def text_img_data_loader(args, data):
    #print(data.keys())
    """
    Parameters
    ----------
    ...
    ----------
    """
    train_dataset = Text_Img_Dataset(
                                data['X_train'][['user_id', 'isbn']].values,
                                data['X_train']['img_vector'].values,
                                data['X_train']['item_summary_vector'].values,
                                data['y_train'].values
                                )
    valid_dataset = Text_Img_Dataset(
                                data['X_valid'][['user_id', 'isbn']].values,
                                data['X_valid']['img_vector'].values,
                                data['X_valid']['item_summary_vector'].values,
                                data['y_valid'].values
                                )
    test_dataset = Text_Img_Dataset(
                                data['text_test'][['user_id', 'isbn']].values,
                                data['img_test']['img_vector'].values,
                                data['text_test']['item_summary_vector'].values,
                                data['text_test']['rating'].values
...
def text_img_data_split(args, data):
    """
    Parameters
    ----------
    Args : argparse.ArgumentParser
    ...
    """
    X_train, X_valid, y_train, y_valid = train_test_split(
                                                        pd.concat((data['text_train'][['user_id', 'isbn', 'item_summary_vector']],data['img_train']['img_vector']),axis=1),
                                                        data['text_train']['rating'],
                                                        test_size=args.test_size,
                                                        random_state=args.seed,
                                                        shuffle=True
