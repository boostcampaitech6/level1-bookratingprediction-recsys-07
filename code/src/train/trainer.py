...
def test(args, model, dataloader, setting):
    ...
    for idx, data in enumerate(dataloader['test_dataloader']):
        if args.model == 'CNN_FM':
            ...
            x, _ = [data['user_isbn_vector'].to(args.device), data['user_summary_merge_vector'].to(args.device), data['item_summary_vector'].to(args.device), data['img_vector'].to(args.device)], data['label'].to(args.device)
        else:
